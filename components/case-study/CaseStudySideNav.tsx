...
export const CaseStudyNav = forwardRef<HTMLDivElement, CaseStudyNavProps>(
  function CaseStudyNav({ sections, activeId, scrollProgress }, ref) {

    return (
      <div ref={ref} className="fixed top-0 left-0 right-0 z-50 flex justify-center px-4 pt-4">
        <div className="flex flex-col items-center">
          {/* Pill navbar */}
          <div className="flex items-center gap-1 rounded-full px-2 py-2 bg-[#0a0a0a]">
            {/* Logo */}
                ...
                return (
                  <a
                    ...
                    className={`text-[12px] px-3 py-1.5 rounded-full transition-all whitespace-nowrap ${
                      isActive ? "text-white bg-white/15" : "text-white/40 hover:text-white/70 hover:bg-white/5"
                    }`}
                  >
                    {section.label}
                  </a>
            ...
            <span className="md:hidden text-[12px] px-3 py-1.5 whitespace-nowrap text-white/60">
